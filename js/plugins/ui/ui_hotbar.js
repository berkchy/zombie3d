...
plugin.register({
  ...
  selectSlot(index) {
    ...
    plugin.emit('hotbar:select', { index: index, slot: this.slots[index] });
  },
  ...
  _renderWeaponIcon(index, weaponId) {
    ...
    var thumb = plugin.get('fx_thumbnail');
    if (!thumb || !thumb.enabled || typeof thumb.getThumbnail !== 'function') return;

    var dataUrl = thumb.getThumbnail(weaponId, wp.modelId, 72);
    if (!dataUrl) return;
