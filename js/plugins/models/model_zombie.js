...
plugin.register({
  ...
  createModel() {
    ...
    armLPivot.name = 'armL';
    armLPivot.position.set(0, -0.04, 0);
    shoulderLPivot.add(armLPivot);

    var armRPivot = new THREE.Object3D();
    armRPivot.name = 'armR';
    armRPivot.position.set(0, -0.04, 0);
    ...
    var upperLegR = new THREE.Mesh(upperLegGeo, pantsMat);
    upperLegR.position.set(0, 0.05, 0);
    upperLegR.castShadow = true;
    upperLegR.name = 'upperLegR_mesh';
    legRPivot.add(upperLegR);

    // ===== DIZ PIVOTLARI =====
    var kneeLPivot = new THREE.Object3D();
    kneeLPivot.name = 'kneeL';
    kneeLPivot.position.set(0, -0.03, 0);
    legLPivot.add(kneeLPivot);

    var kneeRPivot = new THREE.Object3D();
    kneeRPivot.name = 'kneeR';
    kneeRPivot.position.set(0, -0.03, 0);
    ...
    lowerLegL.name = 'lowerLegL_mesh';
    kneeLPivot.add(lowerLegL);

    var lowerLegR = new THREE.Mesh(lowerLegGeo, pantsMat);
    lowerLegR.position.set(0, -0.06, 0);
    lowerLegR.castShadow = true;
    lowerLegR.name = 'lowerLegR_mesh';
    kneeRPivot.add(lowerLegR);

    // ===== BOTLAR =====
    var bootGeo = new THREE.BoxGeometry(0.11, 0.06, 0.16);
    var bootL = new THREE.Mesh(bootGeo, bootMat);
    bootL.position.set(0, -0.09, -0.02);
    bootL.name = 'bootL';
    kneeLPivot.add(bootL);

    var bootR = new THREE.Mesh(bootGeo, bootMat);
    bootR.position.set(0, -0.09, 0.02);
    bootR.name = 'bootR';
    ...
    eyeR.position.set(0.08, 0.02, 0.14);
    eyeR.name = 'eyeR';
    headPivot.add(eyeR);
    ...
    socketR.position.set(0.08, 0.02, 0.09);
    socketR.scale.set(1, 0.5, 0.4);
  ...
  animations: {
    idle: {
      ...
      loop: true,
      tracks: [
        { pivot: 'head', prop: 'rotation.y', keys: [0, 0.4, -0.4, 0, 0, 0] },
        { pivot: 'head', prop: 'rotation.x', keys: [-0.05, -0.1, 0, -0.05, -0.05, -0.05] },
        { pivot: 'armL', prop: 'rotation.x', keys: [-0.3, -0.15, -0.4, -0.2, -0.3, -0.3] },
        { pivot: 'armR', prop: 'rotation.x', keys: [-0.4, -0.25, -0.5, -0.35, -0.4, -0.4] },
        { pivot: 'armL', prop: 'rotation.z', keys: [0.15, 0.08, 0.2, 0.12, 0.15, 0.15] },
        { pivot: 'armR', prop: 'rotation.z', keys: [-0.12, -0.05, -0.18, -0.1, -0.12, -0.12] },
        { pivot: 'torso', prop: 'rotation.z', keys: [0.15, 0.08, 0.2, 0.12, 0.15, 0.15] },
        { pivot: 'torso', prop: 'rotation.x', keys: [0.04, 0.08, 0, 0.05, 0.04, 0.04] }
      ]
    },
    walk: {
      duration: 0.9,
      loop: true,
      tracks: [
        { pivot: 'armL', prop: 'rotation.x', keys: [-0.8, -0.5, -0.8] },
        { pivot: 'armR', prop: 'rotation.x', keys: [-0.4, -0.7, -0.4] },
        { pivot: 'armL', prop: 'rotation.z', keys: [0.15, 0.1, 0.15] },
        { pivot: 'armR', prop: 'rotation.z', keys: [-0.1, -0.05, -0.1] },
        { pivot: 'legL', prop: 'rotation.x', keys: [-0.25, 0.25, -0.25] },
        { pivot: 'legR', prop: 'rotation.x', keys: [0.25, -0.25, 0.25] },
        { pivot: 'kneeL', prop: 'rotation.x', keys: [0.2, 0.35, 0.2] },
        { pivot: 'kneeR', prop: 'rotation.x', keys: [0.35, 0.2, 0.35] },
        { pivot: 'torso', prop: 'rotation.z', keys: [0.15, 0.06, 0.15] },
        { pivot: 'torso', prop: 'rotation.x', keys: [0.05, -0.05, 0.05] },
    ...
    attack: {
      ...
      loop: false,
      tracks: [
        { pivot: 'armL', prop: 'rotation.x', keys: [-0.3, -2.0, -0.5] },
        { pivot: 'armR', prop: 'rotation.x', keys: [-0.4, -2.0, -0.6] },
        { pivot: 'armL', prop: 'rotation.z', keys: [0.15, 0.12, 0.08] },
        { pivot: 'armR', prop: 'rotation.z', keys: [-0.12, -0.12, -0.08] },
        { pivot: 'torso', prop: 'rotation.x', keys: [0.04, -0.15, 0.3] },
        { pivot: 'head', prop: 'rotation.x', keys: [-0.05, -0.1, -0.5] }
      ]
    },
    die: {
      duration: 1.5,
      ...
      tracks: [
        ...
        { pivot: 'kneeL', prop: 'rotation.x', keys: [0, 0.25] },
        { pivot: 'kneeR', prop: 'rotation.x', keys: [0, 0.25] },
        { pivot: 'armL', prop: 'rotation.x', keys: [0.1, -0.35] },
        { pivot: 'armR', prop: 'rotation.x', keys: [-0.3, -0.35] },
        { pivot: 'armL', prop: 'rotation.z', keys: [0, 0.45] },
        { pivot: 'armR', prop: 'rotation.z', keys: [0, -0.45] },
        { pivot: 'head', prop: 'rotation.x', keys: [-0.05, 0.4] }
      ]
    }
  }
});
