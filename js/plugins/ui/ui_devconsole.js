...
plugin.register({
  ...
  init(game) {
    ...
    plugin.on('game:start', this.id, function() {
      if (self._btn) self._btn.style.display = 'flex';
    });
    ...
    console.warn = function() {
      ...
      if (self._originalWarn) self._originalWarn.apply(console, args);
    };
    ...
    console.error = function() {
      ...
      if (typeof crashGame === 'function') {
        crashGame('console', 'error', new Error(msg));
      } else {
  ...
  _registerCommands() {
    ...
    commands.register('ui_devconsole', 'help', function(args) {
      var cmds = commands.getAll();
    ...
    commands.register('ui_devconsole', 'clear', function(args) {
      self._body.innerHTML = '';
      self._logs = [];
      return '';
    }, 'Konsolu temizle');

    commands.register('ui_devconsole', 'disconnect', function(args) {
      if (!window.gameStarted) return 'Oyunda degilsin';
      var g = window.include('game') || window.game;
      if (g && g.disconnect) {
        g.disconnect();
        return 'Oyundan cikiliyor...';
      }
      return 'Hata';
    }, 'Oyundan cik, menuye don');

    commands.register('ui_devconsole', 'plugins', function(args) {
      ...
      if (sub === 'list') {
        ...
        var p = plugin.getAll().filter(function(x) { return x.id === args[1]; })[0];
    ...
    commands.register('ui_devconsole', 'cvar', function(args) {
      ...
      if (sub === 'list') {
        var all = cvar.getAll();
        if (args[1]) {
          var filter = args[1].toLowerCase();
          all = all.filter(function(c) { return c.id.toLowerCase().indexOf(filter) !== -1; });
        }
        if (all.length === 0) return 'Hiç cvar kayıtlı değil' + (args[1] ? ' (filtre: ' + args[1] + ')' : '');
    ...
    commands.register('ui_devconsole', 'sound', function(args) {
      ...
      if (!s) return 'Ses sistemi yok (game.sound undefined)';
      var bank = s._bank || {};
      var sounds = s._sounds || {};
      var bankKeys = Object.keys(bank);
      var soundKeys = Object.keys(sounds);
      var out = '_ready=' + s._ready + ' _unlocked=' + s._unlocked + '\n';
      out += 'Bank (' + bankKeys.length + '): ' + bankKeys.join(', ') + '\n';
      out += 'Sounds (' + soundKeys.length + '): ' + soundKeys.join(', ');
      return out;
    }, 'Ses sistemi durumunu göster');

    commands.register('ui_devconsole', 'eval', function(args) {
      var expr = args.join(' ');
      if (!expr) return 'Kullanım: eval <expr>';
      try {
        ...
        if (typeof result === 'object') {
          ...
          function serialize(val, depth) {
            ...
            if (Array.isArray(val)) {
              return '[' + val.map(function(v) { return serialize(v, depth+1); }).join(', ') + ']';
            }
            var keys = Object.keys(val).slice(0, 15);
            var out = '{';
            keys.forEach(function(k, i) {
              if (i > 0) out += ', ';
              out += k + ': ' + serialize(val[k], depth+1);
            });
            if (Object.keys(val).length > 15) out += ', ...';
            return out + '}';
          }
  ...
  destroy() {
    commands.unregisterAll('ui_devconsole');
