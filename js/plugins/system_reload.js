PluginRegistry.register({
  ...
  init(game) {
    ...
    PluginRegistry.on('weapon:fire', this.id, function(data) {
      if (!self._reloading && data.weapon && data.weapon.ammo !== undefined && data.weapon.ammo <= 0) {
        self._startReload();
      }
    });

    PluginRegistry.on('ammo:change', this.id, function() {
      self._updateReloadBtn();
    });
  },
  ...
  update: function(dt) {
    ...
    if (wp.reloadMode === 'shell') {
      ...
      if (pct >= 1) {
        ...
        PluginRegistry.emit('ammo:change', { ammo: wp.ammo, maxAmmo: wp.maxAmmo, clip: wp.clip });
  ...
  _updateReloadBtn: function() {
    ...
    if (!tb._buttons || !tb._buttons['reload']) {
      tb.touchAdd('reload', {
        ...
        onClick: function() { var r = PluginRegistry.get('system_reload'); if (r && r._startReload) r._startReload(); }
      });
    }
  },
  ...
  destroy: function() {
    ...
    PluginRegistry.off('weapon:fire', this.id);
    PluginRegistry.off('ammo:change', this.id);
    this._hideUI();
  }
});
