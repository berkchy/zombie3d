...
plugin.register({
  ...
  init(game) {
    loader.loadScript('model_pickup_heart', function(){});
    this.game = game;
    this.pickups = [];
  ...
  spawnPickup: function(pos) {
    ...
    var modelPlugin = plugin.get('model_pickup_heart');
    var mesh;
    if (modelPlugin && typeof modelPlugin.createModel === 'function') {
      mesh = modelPlugin.createModel();
    } else {
  ...
  update: function(dt) {
    ...
    for (var i = 0; i < this.pickups.length; i++) {
      ...
      if (dist < 0.8) {
        ...
        if (this.game.sound) this.game.sound.play('pickup');
        plugin.emit('pickup:collect', {
          heal: p.healAmount,
