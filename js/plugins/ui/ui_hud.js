...
plugin.register({
  ...
  type: 'ui',
  version: '1.0',
  description: 'Can ve puan gostergesi',
  priority: 50,
  enabled: true,
  ...
  init(game) {
    ...
    var self = this;
    plugin.on('game:start', this.id, function() { c.classList.add('show'); });
    plugin.on('game:over', this.id, function() { c.classList.remove('show'); });
  },
